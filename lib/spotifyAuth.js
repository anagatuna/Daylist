...
async function getStoredTokens(uid) {
  const raw = await SecureStore.getItemAsync(tokensKey(uid));
...
async function saveTokens(uid, tokenResponse) {
  ...
  await SecureStore.setItemAsync(tokensKey(uid), JSON.stringify(tokens));
  return tokens;
}
...
export async function disconnectSpotify(uid) {
  await SecureStore.deleteItemAsync(tokensKey(uid));
  await SecureStore.deleteItemAsync(profileKey(uid));
}
...
export async function fetchSpotifyProfile(uid) {
  ...
  await SecureStore.setItemAsync(profileKey(uid), JSON.stringify(profile));
  return profile;
}

export async function getCachedProfile(uid) {
  const raw = await SecureStore.getItemAsync(profileKey(uid));
  return raw ? JSON.parse(raw) : null;
}
